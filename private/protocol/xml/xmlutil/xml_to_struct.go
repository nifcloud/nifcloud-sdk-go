...
func StructToXML(e *xml.Encoder, node *XMLNode, sorted bool) error {
	err := e.EncodeToken(xml.StartElement{Name: node.Name, Attr: node.Attr})
	if err != nil {
		return err
	}

	if node.Text != "" {
		err := e.EncodeToken(xml.CharData([]byte(node.Text)))
		if err != nil {
			return err
		}
	} else if sorted {
	...
	err = e.EncodeToken(xml.EndElement{Name: node.Name})
	if err != nil {
		return err
	}
	return e.Flush()
